...
class YModem(object):
    def __init__(self, getc, putc, header_pad=b'\x00', data_pad=b'\x1a'):
        ...
        dev=''
        if self.pcname.find('Y'):
            self.ser=serial.Serial('/dev/serial/by-id/usb-fusionRobotics_D1_b0001-if00-port0',115200,timeout=1)
        else:
            dev='/dev/serial/by-id/'+os.listdir('/dev/serial/by-id/')[0]
