...
class YModem(object):
    def __init__(self, header_pad=b'\x00', data_pad=b'\x1a'):
        self.ser=serial.Serial('/dev/serial/by-id/usb-FTDI_FT230X_Basic_UART_DK0B1IJL-if00-port0',115200,timeout=1)
        self.getc = self.getc
        self.putc = self.putc
        self.st = SendTask()
        self.rt = ReceiveTask()
    ...
    def crc_g0(self,command):
        crc_func = crcmod.mkCrcFun(0x11021, rev=False, initCrc=0x0000, xorOut=0x0000)
        crc = struct.pack('H', crc_func(command))
        return crc
    def gen_frame(self,addr,cmd='R'):
        command=addr+cmd
        command = command.encode('latin-1')
        command +=self.crc_g0(command)
    ...
    def putc(self,data):
        #ser=serial.Serial('/dev/serial/by-id/usb-fusionRobotics_D1_b0001-if00-port0',115200,timeout=1)
    ...
    def send_file(self, file_path, retry=20, callback=None):
        try:
            ...
            self.log.info('file_name--->%s'%file_size)
        ...
        if self.getc(1)!=CRC:
            ...
            if self.stop_download()==True:
                print('download failed,stop success')
            else:
                time.sleep(1)
                rev=self.getc(1)
                print('rev')
    ...
    def send(self, data_stream, data_name, data_size, retry=20, callback=None):
        ...
        while True:
            ...
            #self.log.info('_make_send_checksum')
            checksum = self._make_send_checksum(data)
        ...
        # [Final packet >>>]
        header = self._make_edge_packet_header()
        data = "".ljust(128, self.header_pad)#bytes.decode(self.header_pad))
        checksum = self._make_send_checksum(data)
        data_for_send = header + data.encode() + checksum
        self.putc(data_for_send)
        self.st.inc_sent_packets()
    ...
    def reset_pcb(self,addr,cmd):
        data=self.gen_frame(addr,cmd)
        self.putc(data)
        time.sleep(0.2)
        rev=self.ser.read(self.ser.inWaiting())
        print('rev--->%s'%rev)
        return True
    def run_to_download(self):
        ...
        while time.time()-start_time<5:
            ...
            time.sleep(0.2)   
            cnt+=1
            print('run to download cnt--->%d'%cnt)
            if self.wait_for_next(CRC)==True:
                return True
        return False
def usage():
    print '\n\tUSAGE: %s <ADDR> <ROM-FILE>\n\n' %(sys.argv[0])
    
def main():
    if len(sys.argv) != 3:
        usage()
        sys.exit(-1)
    #elif sys.argv[1] not in DEVICES_ADDR:
    #    print '\t\nUNKNOW DEVICE ADDR(%s)!' %sys.argv[1]
    #    sys.exit(-1)
    elif not os.path.isfile(sys.argv[2]):
        print '\t\nCANNOT OPEN ROM FILE(%s) !' %sys.argv[2]
        sys.exit(-1)
    else:
        addr = sys.argv[1]
        rom_file = sys.argv[2]
    ...
    print('---------------------------')
    ymodem=YModem()
    if ymodem.reset_pcb(addr,'R') is not True:
        print '\n\t CAN OPEN DEVICE FOR FLASH...CHECK!\n'
        sys.exit(-2)
    else:
        print '\n\treset success\n'
    
    if ymodem.run_to_download() is not True:
        sys.exit(-2)
        print('\n\t CAN NOT RUN TO DOWNLOAD')
    print('\n\t device is ready to flash')
    curDir = os.path.abspath(os.path.dirname(__file__))
    binPath = os.path.join(curDir, rom_file)
